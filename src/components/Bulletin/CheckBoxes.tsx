import { tags, canAccessRecruiterData } from "@lib/constants";
    ...
    <form className="p-3 w-full flex gap-2 min-h-[2.25rem] flex-1 min-w-0">
      {!isRecruiterPortal || hasRecruiterAccess ? (
        <>
          <input
            type="Search"
            placeholder={hasRecruiterAccess ? "Search candidates..." : "Search..."}
            onChange={(e) => {
              ...
              >
                <div className="lg:grid lg:grid-cols-[max-content_max-content] flex flex-col gap-2 lg:h-[40vh] h-[70vh] max-w-[200px] md:max-w-none">
                  ...
                  <div className="h-full overflow-y-scroll">
                    {hasRecruiterAccess ? <MajorsCheckboxes /> : <OrgsCheckboxes />}
                  </div>
                </div>
              </div>
            </div>
      ...
      {!isRecruiterPortal && User?.role !== "company" && (
